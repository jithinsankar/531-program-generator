...
export default function ProgramGenerator() {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      const { maxLifts: savedMaxLifts, enabledLifts: savedEnabledLifts, unit: savedUnit } = JSON.parse(savedData)
  ...
  const saveToLocalStorage = () => {
    localStorage.setItem('531ProgramData', JSON.stringify({ maxLifts, enabledLifts, unit }))
  }
  ...
  const toggleUnit = () => {
    ...
    setMaxLifts(prev => {
      ...
      return Object.fromEntries(
        Object.entries(prev).map(([lift, weight]) => [lift, Math.round(weight * conversionFactor)])
      )
    })
  }
  ...
  const generateProgram = () => {
    ...
    Object.entries(maxLifts).forEach(([lift, oneRepMax]) => {
      ...
      const nextCycleIncrease = Math.round(trainingMax * INCREASE_PERCENTAGE / (unit === 'lbs' ? 5 : 2.5)) * (unit === 'lbs' ? 5 : 2.5)
  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl">
      <Card>
        ...
        <CardContent>
          <form onSubmit={(e) => { e.preventDefault(); generateProgram(); }} className="space-y-4">
            ...
            {Object.keys(maxLifts).map((lift) => (
              <div key={lift} className="flex items-center space-x-2">
                ...
                <div className="flex-grow">
                  <Label htmlFor={lift}>{lift} 1RM ({unit})</Label>
                  <Input
                    id={lift}
                    type="number"
                    value={maxLifts[lift] || ''}
                    onChange={(e) => handleInputChange(lift as Lift, e.target.value)}
            ...
            <Button type="submit" className="w-full">Generate Program</Button>
          </form>
        </CardContent>
      </Card>
        ...
        <div ref={programRef}>
          <Card className="mt-8">
            <CardHeader>
              ...
              <CardDescription>4-week cycle based on your one-rep max lifts (weights in {unit})</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
